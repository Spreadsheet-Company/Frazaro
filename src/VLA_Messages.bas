...
Private Sub AddEntries(ByVal m As Collection)
    ...
    AddMsg m, "interp-dynamicget-neither", 5, "VLA-Interpreter", "IN.2: '{member}' is neither a readable property nor a callable method on this object (Get: {getErr}; Method: {methodErr})"
    AddMsg m, "interp-dynamiccall-neither", 5, "VLA-Interpreter", "IN.2: '{member}' could not be called as a method or read as a property on this object (Method: {methodErr}; Get: {getErr})"
    AddMsg m, "interp-dynamicset-failed", 5, "VLA-Interpreter", "IN.2: '{member}' could not be assigned (Let: {letErr}; Set: {setErr})"
    AddMsg m, "interp-dispatch-too-many-args", 5, "VLA-Interpreter", "IN.2: this interpreter's dynamic dispatch supports up to 4 positional arguments per call; '{member}' was called with {n}"
