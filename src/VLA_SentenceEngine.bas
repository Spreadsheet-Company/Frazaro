...
' tools/check_rule_coverage.ps1 verifies: "XXXXXXXX over N non-whitespace
' bytes". XXXXXXXX is a 32-bit hash - h = (h * 31 + b) mod 2^32 over every
' byte b of the file that is not a tab, LF, CR or space - rendered as eight
' uppercase hex digits; N is how many bytes took part, a human-readable
' second opinion. Deliberately simple enough to be written identically in
' two languages by hand: the PowerShell twin is Get-SourceHash in the
' checker, and the two MUST stay byte-for-byte equivalent (same skip set,
' same multiplier, same modulus). Whole-file bytes, never text: a
' UTF-8 BOM or a non-ASCII character hashes the same on both sides
' because neither side decodes anything. Double arithmetic because VBA
' has no unsigned 32-bit integer: h * 31 + 255 stays below 2^37, well
' inside a Double's exact-integer range, and Hex$ of the value re-cast
' as a signed Long prints the same eight digits an unsigned view would.
Public Function EnglishSourceHash(ByVal filePath As String) As String
    ...
    End If
    Close #f
    Dim h As Double, counted As Long, i As Long, b As Long
    For i = 0 To n - 1
        b = buf(i)
    ...
    If h >= 2147483648# Then
        hx = Hex$(CLng(h - 4294967296#))
    Else
        hx = Hex$(CLng(h))
    End If
    EnglishSourceHash = Right$("00000000" & hx, 8) & " over " & counted & " non-whitespace bytes"
End Function
