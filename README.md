...
gathered into a list — written `(list a b c)` — and that list can then be
counted, added up, searched or taken apart, with `length`, `member`,
`nth`, `append`, `reverse` and `sum-list`. Negation-as-failure, arithmetic via
